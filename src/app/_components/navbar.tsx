...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 z-50 flex h-20 w-full items-center justify-between gap-4 border-b bg-[#F8F8F8] px-4 md:px-6">
      ...
      <nav className="hidden flex-col gap-6 font-medium md:flex md:flex-row md:items-center md:gap-5 md:text-sm lg:gap-10">
        {navLinks.map((link) => (
          <Link
            key={link.name}
            href={link.href}
            className="text-black transition-colors hover:font-bold hover:text-neutral-700"
          >
            {link.name}
          </Link>
        ))}
      ...
      <Sheet>
        <SheetTrigger asChild>
          <Button
            size="icon"
            className="shrink-0 border border-neutral-700 md:hidden"
          >
            <Menu />
